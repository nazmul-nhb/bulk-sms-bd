...
The library is configured to export multiple paths via `package.json`. You can import the specific module(s) you need:
...
| `bulk-sms-bd/types`     | TypeScript interfaces and types.                   | `import type { Message } from 'bulk-sms-bd/types'`    |
...
The primary class used to interact with the bulksmsbd.net API.
...
The configuration object passed to the constructor `new BulkSmsClient(configs)` is validated strictly to ensure all required API parameters are set:

- It cannot be an empty object (validated using [`isEmptyObject`](https://toolbox-x.nazmul-nhb.dev/docs/guards/misc/object#isemptyobject)).
- It must contain the keys `bulkSmsApiKey` and `bulkSmsSenderId` (validated using [`isObjectWithKeys`](https://toolbox-x.nazmul-nhb.dev/docs/guards/misc/object#isobjectwithkeys)), and both must be non-empty strings ([`isNonEmptyString`](https://toolbox-x.nazmul-nhb.dev/docs/guards/primitive/composite#isnonemptystring)).
- If the optional `bulkSmsBaseApi` property is provided, it must also be a non-empty string.

Any failure in configuration validation throws a `BulkSmsError` with code `1003`.
...
Any text input—including configuration keys, message text, and endpoints—is strictly validated to ensure it is of type `string` and is not empty (checked via [`isNonEmptyString`](https://toolbox-x.nazmul-nhb.dev/docs/guards/primitive/composite#isnonemptystring)). Null, undefined, empty strings, or non-string values will trigger input validation failures and throw `BulkSmsError` with code `1003`.
